...
def mapper(url_article):
    ...
    """
    ua_dict=json.loads(url_article)
    url = ua_dict.keys[0]
    article = ua_dict.keys[url]
    words_dict = {}
...
if __name__ == "__main__":
    ...
    input_file_path = "hdfs://localhost:9000/user/user/wiki_index.tb"
    output_file_path = "hdfs://localhost:9000/user/spark_output.tb"
    
    if exists(output_file_path):
        rm(output_file_path, recurse=True)

    sc = SparkContext()
    input_hdfs_file = sc.textFile(input_file_path)
